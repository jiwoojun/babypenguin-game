...
def makeAccessary(image2: Image, name: str):
    global newItem
    newItem = sprites.create(image2, SpriteKind.accessary)
    sprites.set_data_string(newItem, "name", name)
    accessaries.append(newItem)

def on_create_renderable(screen2):
    global index, item_top
    screen2.fill_rect(72, 5, 85, 110, 14)
    screen2.draw_rect(72, 5, 85, 110, 15)
    index = 0
    item_top = 28
    while index <= len(accessaries) - 1:
        spriteutils.draw_transparent_image(accessaries[index].image, screen2, 14 + index * 20, item_top)
        index += 1
spriteutils.create_renderable(0, on_create_renderable)

item_top = 0
index = 0
accessaries: List[Sprite] = []
newItem: Sprite = None
